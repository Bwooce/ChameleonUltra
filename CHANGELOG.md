...
## [unreleased][unreleased]
 ...
 - Fixed the device resetting when a contactless payment card was presented. The ISO 14443-4 reassembly loop only exited on a timeout, so a card whose exchanges all succeeded while the chaining bit stayed set looped until the watchdog fired. Reproducible at exactly 5 s with contactless Visa and Mastercard. Both reassembly loops are now bounded by block count and by total wall-clock
 - Fixed R(ACK) carrying the wrong block number during a chained response. It echoed the received I-block's number, but ISO 14443-4 advances the card's chain only when the number differs, so cards retransmitted the same block indefinitely instead of continuing
 - Fixed the reader block number being toggled once per APDU rather than once per received I-block. A chained response of even length left the session out of step, and the card answered the next command by retransmitting its previous response
 - Fixed Start-up Frame Guard Time not being honoured after RATS. A card requesting SFGT silently ignored the first frame, after which every APDU timed out while the card still answered R(NAK) normally -- presenting as "card present but mute". Observed on a contactless Mastercard
 - Fixed `hf 14a` APDU exchanges not handling S(WTX): a card asking for more time received an R(ACK) instead of a WTX response, and its timing byte was returned as though it were card data
 - Fixed incomplete ISO 14443-4 chained responses being reported as success. A chain broken by a timeout, a CRC failure or the block limit returned the partial data with an OK status, so the host parsed the middle of a response as its status word
 - Fixed emulated tags mis-reassembling chained commands from a reader: each fragment overwrote the previous one instead of appending, and the acknowledgement carried a stale block number. Note that card-to-reader chaining is still not implemented, so an emulated tag can over-run a reader that advertises a small frame size
